...
class State(TypedDict):
    ...
    query_type: str        # 'factual' or 'experiential'
    query: str             # SQL query
...
###############################################################################
analysis_system_prompt = """Analyze the user question and determine whether it is asking for factual (structured data) information or experiential (unstructured, descriptive) information. Answer with a single word: 'factual' or 'experiential'. """
...
def analyze_question(state: State) -> dict:
    ...
    try:
        ...
        query_type = "experiential" if "experiential" in result.lower() else "factual"
...
- Always the preference is to do vector search in the vector_embeddings table, if necessary combine the vector_embeddings table with travel_plans with the key id. 
- Avoid filtering as ILIKE or LIKE.
- Use 'travel_plans' For factual questions such as counts, means or other metrics in columns like org, dest, days.
- For experiential or descriptive queries (such as recommendations or experiences), always preferably use 'vector_embeddings' via semantic vector search.
- IMPORTANT: If you receive a query that includes a location or similar filter (for example, “restaurants in chicago”), you must first query the 'travel_plans' table to extract the relevant travel plan IDs that mention the location (e.g., chicago). Then, use those IDs to run a semantic search on the 'vector_embeddings' table. Do not apply the location filter directly to the vector_embeddings table since semantic search is used to handle that.
- If the question involves both factual and experiential information, use a hybrid approach (combining both sources as needed).

The SQL dialect will be always PostgreSQL for supabase.
Use the relevant information provided below to inform your SQL.
...
def check_query(state: State) -> dict:
    logging.info("==== check_query: Start ====")
    try:
        ...
        logging.info("==== check_query: Success - Query Validated ====")
        return {
            "query": query_only,
            "validation_details": validated_query
        }
    except Exception as e:
        logging.error("==== check_query: Error: %s ====", str(e), exc_info=True)
        raise e
...
def execute_query(state: State) -> dict:
    ...
    try:
        ...
        logging.error("==== execute_query: Error: %s ====", str(e), exc_info=True)
        raise e
...
def generate_answer(state: State) -> dict:
    ...
    try:
        if state.get("query_type") == "experiential":
            prompt = (
...
try:
    # Función execute_query actualizada para usar psycopg2 directamente
    def execute_query(state: State) -> dict:
        logging.info("==== execute_query: Start ====")
        try:
            query = state["query"]
            
            # Si la consulta usa vector (se detecta "::vector")
            if "::vector" in query:
                # Generar el embedding para la pregunta original
                embedding = generate_embedding(state["question"])
                # Convertir el embedding a cadena para vector casting
                ...
                # Según el estilo de binding de parámetros, se ejecuta de manera adecuada
                if "$1::vector" in query:
                    # Reemplazar el binding servidor ($1) por el estilo client-side (%s)
                    ...
                    # Fallback: reemplazar directamente el placeholder
                ...
                # Para consultas que no requieren vector, usar también psycopg2
                USER = os.getenv("DB_USER")
                PASSWORD = os.getenv("DB_PASSWORD")
                HOST = os.getenv("DB_HOST")
                PORT = os.getenv("DB_PORT")
    ...
    # Wrapper para la función write_query que asegura el uso de %s en vez de $1
    ...
    # Función para analizar el resultado de la consulta
    ...
    graph_builder.add_node("write_query", write_query_wrapper)  # Usamos el wrapper para corregir placeholders
    graph_builder.add_node("execute_query", execute_query)
    graph_builder.add_node("execute_vector_query", execute_vector_query)
    graph_builder.add_node("analyze_query_result", analyze_query_result)
    graph_builder.add_node("generate_answer", generate_answer)
    
    # Definir las aristas del grafo
    graph_builder.add_edge(START, "analyze_question")
    graph_builder.add_edge("analyze_question", "write_query")
    
    # Ruta condicional según el tipo de consulta (factual o vectorial)
    ...
    graph_builder.add_conditional_edges("write_query", path=named_route_execution)
    
    graph_builder.add_edge("execute_query", "analyze_query_result")
    graph_builder.add_edge("execute_vector_query", "analyze_query_result")
    
    graph_builder.add_edge("analyze_query_result", "generate_answer")
    
    # Compilar el grafo
    graph = graph_builder.compile()
    
    # (Opcional) Generar un diagrama Mermaid
...
# is properly connected in the graph sequence (it currently is) and that your database and vector store are accessible.